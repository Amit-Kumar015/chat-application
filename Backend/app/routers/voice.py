...
async def voice_chat_turn(session_id: str = Form(...), file: UploadFile = File(...)):
  db = get_database()
  ...
  try:
    ...
    if not existing_session:
      ...
      await db.sessions.update_one({"session_id": session_id}, {"$set": {"updated_at": now}})
      
    retriever = get_retriever(session_id=session_id, k=4)
    retrieved_docs = await retriever.ainvoke(user_prompt)
    chat_history = await get_recent_chat_history(session_id=session_id, limit=6)
    ...
    await db.chat_history.insert_many([
      {"session_id": session_id, "role": "user", "content": user_prompt, "timestamp": now},
      {"session_id": session_id, "role": "assistant", "content": bot_reply, "timestamp": datetime.now(timezone.utc)}
    ])
